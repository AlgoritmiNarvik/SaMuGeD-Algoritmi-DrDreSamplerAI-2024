...
def do_stuff(i, n_threads, queue):
    k = 0
    while i*k <= len(queue):
        task = queue[k*n_threads + i]
        task.start()
        task.join()
        k+=1
...
def folder():
    ...
    if input_path != "":
        ...
        for rootdir, dirs, files in os.walk(input_path):
            ...
            if i == 2200: #limit amount of subfolders to iterate through
                break
            i+=1
        ...
        for i in range(max_thread_count):
            worker = threading.Thread(target=do_stuff, args=(i, max_thread_count, process_queue))
            worker.start()
