    ...
    <script>
        ...
        function updateSegmentOutput(segments) {
            ...
            segments.forEach((seg, index) => {
                outputDiv.innerHTML += `<p>Segment ${index + 1}: Start: ${seg.start.toFixed(2)}s, End: ${seg.end.toFixed(2)}s, Duration: ${(seg.end - seg.start).toFixed(2)}s</p>`;
            });
        }
    </script>
</body>
...
def segment_midi():
    ...
    try:
        ...
        return jsonify({
            'plots': plots,
            'segments': segments,
            'midi_segments': [segment.write(io.BytesIO()).getvalue() for segment in midi_segments]
        })
    ...
    try:
        ...
        with open(os.path.join(temp_dir, 'segments_summary.txt'), 'w') as f:
            for i, segment in enumerate(data['segments']):
                f.write(f"Segment {i+1}: Start: {segment['start']:.2f}s, End: {segment['end']:.2f}s, Duration: {segment['end']-segment['start']:.2f}s\n")
